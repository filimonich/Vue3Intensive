<template>
  <div v-if="hasProduct">
    ...
    <div class="row" v-if="canSetRating">
      <div class="col col-2">
        <div>
          <small>Your last mark: {{ userMark.last || "none" }} </small>
        </div>
        <div>
          <small>Your current mark: {{ userMark.current || "none" }}</small>
        </div>
      </div>
      <div class="col col-8">
    ...
    <p>
      Qui reprehenderit quae, ipsam odio tempore minima molestias placeat vel,
      eius quidem itaque assumenda sed dolores a commodi, quibusdam fuga eveniet
      cum. Doloremque, assumenda rem. Vel perferendis architecto, ab magnam.
    </p>
  </div>
  <app-404 v-else />
</template>
...
export default {
  ...
  computed: {
    ...
    canSendMark() {
      return (
        !this.userMark.pending && this.userMark.current != this.userMark.last
      );
    },
  },
  methods: {
    ...
    async sendMark() {
      if (this.canSendMark) {
        ...
        let { res, data } = await this.$api.products.mark(
          this.id,
          this.userMark.current
        );
